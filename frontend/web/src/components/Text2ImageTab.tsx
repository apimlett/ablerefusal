...
export default function Text2ImageTab({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const params = {
      ...
      width,
      height,
      steps: fastMode ? Math.min(steps, 20) : steps,
      cfg_scale: cfgScale,
      seed,
      sampler: fastMode && sampler !== 'LCM' ? 'LCM' : sampler,
      batch_size: 1,
      model: currentModel,
    };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-between p-4 bg-palenight-bgDark rounded-lg border border-palenight-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-palenight-text">Fast Mode</div>
            <div className="text-sm text-palenight-comment">20 steps, better quality</div>
          </div>
        </div>
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          />
          <div className="w-11 h-6 bg-palenight-highlight rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-palenight-purple"></div>
        </label>
      </div>
